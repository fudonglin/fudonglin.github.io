...
What’s striking is that all of these words share the same subword: `merc`. As we’ll see later, this kind of meaningful subword structure can emerge naturally—even without any linguistic rules—when we apply frequency-based tokenization algorithms like GPT‘s **Byte Pair Encoding (BPE)**.



> 🔍  If you’d like to implement BPE from scratch, you can find a simple GPT-2 style implementation here: [Byte Pair Encoding (BPE) Tokenizer — From Scratch (GPT-2 Style)](https://github.com/fudonglin/blog_code_repository/tree/main/bpe).
...
Our goal is to show how BPE can automatically discover a **meaningful subword**, such as `merc`, **purely from statistics**—without any linguistic rules or prior knowledge.
...
| Word        | Byte-Level Token          | Token ID                                                    |
| ----------- | ------------------------- | ----------------------------------------------------------- |
...
- [MiniBPE](https://github.com/karpathy/minbpe), a clean and well-documented BPE implementation by Andrej Karpathy
